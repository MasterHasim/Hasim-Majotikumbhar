...
**Still needs one thing from you: a real access token.** The whole pipeline (Admin config → Dashboard fetch → backend → Meta Graph API call) is wired, tested, deployed, and confirmed live — right now it correctly shows "Meta Ads is not configured yet" until `META_ACCESS_TOKEN` is set, rather than pretending to have data. `MetaAdsProvider`'s request/response handling is UNVERIFIED against a real Meta response (modeled on documented shape only) — the first real fetch will confirm/fix it, same as how ExotelProvider/ExotelVoiceProvider started out.

10 new backend regression tests (183 total, all passing), clean typecheck, deployed, live-verified — real "Entartica Sea World" account registered through the actual Admin UI, Dashboard correctly shows the account picker and the exact expected configuration error.
