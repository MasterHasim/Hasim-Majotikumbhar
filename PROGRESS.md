...
- **Confirm sent replies now show up in the thread.** You reported a sent reply not appearing in the portal even though the customer received it — likely caused by a cross-request cache I'd just added. Reverted it (deployed @35). Please send a test reply and confirm it appears immediately; if it still doesn't, tell me and I'll dig further (would mean the cause is something else, e.g. Sheets' own timing).
- **Media sending is still not live-verified end-to-end** — attaching from a local file now works UI-wise, but whether Exotel actually delivers it is untested. If a media send fails, check Apps Script → Executions for `sendMediaReply` and tell me whether it errors, hangs, or sends-but-never-arrives.
- **Clarify "Previous Conversations (0)"** for a customer like Eva — is that customer known to have older/closed conversations that should be listed there, or is 0 accurate (no prior conversations yet)?
