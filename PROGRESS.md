...
**Deliberately not done yet, per your own constraint**: no existing number/bot has been migrated onto this system — every currently-live chatbot keeps using the original single-bot fields/routes untouched. Migrating is a separate, later, explicitly-gated phase (Phase 2 of the approved plan) — ask when you're ready to start moving a real bot over. **Still needs your live click-through** — I built and deployed this and it's fully covered by automated tests through the real ingestion path, but actually clicking Admin → Chatbot Profiles, adding a profile, and generating a key needs you signed in; I can't complete Google sign-in myself.
