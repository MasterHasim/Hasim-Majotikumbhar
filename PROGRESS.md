...
- **All "Edit" buttons across the admin pages now use a proper card/dropdown modal instead of sequential prompt() popups (@44)** — Users, Customer details, Quick Replies, Team Members, WhatsApp Numbers, Lead Stages. Please click through each once to confirm they save correctly. Three lower-priority prompt()s remain (snooze duration, template variables, submit-for-review wabaId) — not "edit a record" flows, left for now unless you want those converted too.
