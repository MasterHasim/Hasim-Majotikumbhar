...
## 🟡 Multi-bot-per-number, Phase 2: real bot migration prepared, waiting on your bot team (2026-09-01)

**Per your "let's build Phase 2 — migrating a real bot onto it."** Checked live data first to find the actual target: **Entartica CRM (079-485-02801)** is the only number with the old single-bot system genuinely active — `chatbotMode: active`, webhook `https://d3lavo3cdbyj36.cloudfront.net/webhooks/echt-connect/inbound`, and a real `REPLY_SENT` from ~90 minutes before I checked. This is your real FastAPI bot, live, answering real customers right now — so I treated the actual cutover (flipping that number off the old system) as the one step that needs your explicit go-ahead, not something to do silently, per the plan's own "real coordination, not a silent backend migration" note.

**What's done and 100% safe so far (zero effect on the live bot — confirmed `chatbotMode` is still `active` after all of this)**: created a new `ChatbotProfile` on Entartica CRM ("Entartica CRM Bot (migrated from single-bot)"), carrying over the same webhook URL and external profile id (`entartica-coimbatore-pontoon`) the old system uses, mode `active`, priority 0. This is inert until the number's `chatbotMode` is switched off — the dispatch guard added in Phase 1 makes sure of that.

**What's blocked on you / your bot team, and why I didn't push further myself**: the new profile needs its own API key + webhook secret, and I generate those through the exact same one-time-reveal flow as the Chatbot Profiles UI — meaning I deliberately can't see or relay the plaintext myself (the browser tooling actively blocks a raw secret from reaching my output, and I'm not going to route around that). **Please go to Admin → Chatbot Profiles → Entartica CRM → click "Rotate key" on "Entartica CRM Bot (migrated from single-bot)"**, copy the API key, webhook secret, and reply-callback URL shown, and hand them to whoever maintains the FastAPI bot. They need to: (1) send replies to the new `.../api/integrations/chatbot/profiles/<profileId>/reply` URL instead of the old `.../numbers/<numberId>/reply` one, using the new API key as the Bearer token, and (2) verify inbound webhook signatures with the new webhook secret instead of the old one. The inbound webhook URL itself is unchanged (same CloudFront endpoint) — only the reply-callback and its credentials are new.

**Once the bot team confirms they're pointed at the new URL/key**, tell me and I'll flip `chatbotMode` to `off` for Entartica CRM (Admin → Numbers, one dropdown) — that single flag flip is the actual cutover: it simultaneously stops the old system and activates the new profile dispatcher for that number, with no gap in between since the new profile is already fully configured and waiting. I'll watch the activity log right after to confirm the first real message routes through the new path with a real `WEBHOOK_SENT`, before calling this done.

## ✅ Multi-bot-per-number chatbot support — Phase 1, fully additive (2026-09-01)

**Per your explicit request**: "in future there will be multiple chat bots will be needed to connect... how can multiple bots deploy to that dedicated number" — with your explicit constraint "Multiple bots on the same number, but for now do not change current stacks once this get build then replace" and "Priority order, admin-set" for routing. Planned in Plan Mode, approved, then built exactly to that plan.

**What it does**: a WhatsApp number can now have several independent bot **profiles** (e.g. a Sales Bot and a Support Bot both on one number) instead of just one. New Admin → **Chatbot Profiles** tab: pick a number, add profiles, reorder them (drag priority via ↑/↓), set each one's mode (off/shadow/active/paused), and generate a per-profile API key + webhook secret — same one-time-reveal UX as the existing single-bot Chatbot tab. A new conversation is routed to the highest-listed Active/Shadow profile; once routed, that conversation **sticks with that exact profile** even if the order changes later or a higher-priority profile is added afterward — no bot-switching mid-conversation. New `POST /api/integrations/chatbot/profiles/:profileId/reply` callback route, keyed by profile, with a defense-in-depth check: even a bot holding a technically-valid key for *some* profile on that number gets a 409 if the conversation was actually routed to a *different* profile.

**Zero risk to the live single-bot system, by construction, not just discipline**: the new dispatcher (`dispatchInboundMessageToChatbotProfiles`) is called once, right after the existing chatbot code in `Phase4Api.ingestInboundMessage`, and its very first line is `if (number.chatbotMode && number.chatbotMode !== 'off') return;` — so any number still using the old single-bot fields is a guaranteed no-op for the new system, and a number with zero profiles configured costs one empty list-read and nothing else. Proved this with a dedicated coexistence test: a number left on the old `chatbotMode: 'active'` with a stray `ChatbotProfile` record accidentally attached to it — confirmed only the old webhook fires, the new one never does, and `conversation.chatbotProfileId` is never touched.

New backend files: `chatbotProfileRouting.ts` (pure priority-order + sticky-assignment decision function, mirrors the existing `chatbotRouting.ts`'s "decide, don't act" shape) and `chatbotProfileApi.ts` (CRUD + outbound webhook delivery + inbound reply callback, mirrors `chatbotIntegrationApi.ts` 1:1 but scoped to a profile instead of a number — one credential *per profile*, not per number, so a leaked key only ever compromises one bot). 15 new tests (8 pure routing-logic, 7 integration through real `Phase4Api.ingestInboundMessage`) covering priority selection, sticky assignment surviving a reorder, cross-profile key rejection (401), wrong-profile reply rejection (409 `CHATBOT_WRONG_PROFILE`), per-profile webhook signing, shadow/handover parity with the single-bot system, and the coexistence guarantee above. 306 backend tests total, all passing; frontend typecheck + build clean. Deployed both backend and frontend.

**Deliberately not done yet, per your own constraint**: no existing number/bot has been migrated onto this system — every currently-live chatbot keeps using the original single-bot fields/routes untouched. Migrating is a separate, later, explicitly-gated phase (Phase 2 of the approved plan) — ask when you're ready to start moving a real bot over.

**Live-verified after you signed in — and found + fixed one real, pre-existing bug along the way**: clicking into Admin crashed immediately (blank screen) before even reaching the new tab — `TypeError: Cannot read properties of undefined (reading 'includes')` in the Users tab's role-checkbox rendering. Root cause: `u.roleIds.includes(r.id)` assumed every user record has a `roleIds` array, but Firebase RTDB silently drops empty arrays on write (the same known quirk documented elsewhere in this project, e.g. the 2026-08-21 Teams bug) — a user with zero roles has no `roleIds` field at all once read back, not `roleIds: []`. This predates and is completely unrelated to the chatbot work; it just happened to block the very next thing you tried to click. Fixed both `UsersTab` call sites defensively (`u.roleIds ?? []`), redeployed, confirmed Admin loads cleanly. Then live-tested the actual new feature end to end on a real (throwaway) profile: added "Test Sales Bot" to Entartica - Prayagraj, generated a real key/webhook secret pair, confirmed the reply-callback URL and both `echtcbp_`/`echtcbpwh_`-prefixed credentials render correctly, then deactivated it again (mode was left "off" throughout, so it never routed anything). Everything worked first try.

## ✅ Strict app audit + fixed 6 of its top-10 findings, plus test infra and code-splitting (2026-08-24)

**Per your explicit "Rate our app strictly, and based on rating work on missing points, make a summary."** Ran a from-scratch, evidence-based audit (not a self-report — every claim checked against actual current code, not doc comments or this file's own history) across 7 dimensions. Scores: **Security 6/10, Data integrity 6/10, Test coverage 7/10 backend / 0/10 frontend, Performance 6/10, UX completeness 7/10, Code quality 6/10, Documentation 4/10.**

**What it found, and what I fixed live tonight (6 of the top 10, ordered cheapest-to-highest-value as the audit itself ranked them):**

1. **No `deploy` command for the frontend** — `webapp/frontend/package.json` had `build`/`preview`/`typecheck` but nothing to actually ship it; a developer had to reverse-engineer the right `wrangler deploy` invocation from `wrangler.toml`'s presence. Added `npm run deploy` (build + `wrangler deploy`) — used it for real tonight, confirmed it works end to end.
2. **A missed N+1, sitting right next to two already-fixed ones.** `Phase13Api.searchConversations`'s `stageId` filter (`phase13Api.ts:69-76`) was still doing one `customerStages.get()` per conversation — the exact bug class fixed twice already in the same function (query search, number lookups) just wasn't caught here. Memoized it the same way; added a regression test that proves it (two conversations sharing one customer via two different numbers, asserting exactly 1 Firebase call instead of 2).
3. **`.dev.vars.example` only documented 7 of the 15 env vars the code actually reads** — missing `ALLOWED_ORIGINS`, `FIREBASE_DATABASE_URL`, `FRONTEND_URL`, `META_ACCESS_TOKEN`, `RESEND_API_KEY`, `RESEND_FROM_EMAIL` (plus a note that `MEDIA_BUCKET` is an R2 binding in `wrangler.toml`, not a var at all). A fresh local setup would silently misconfigure CORS/emails/ads with no clue why. Filled in all of them with real explanations.
4. **`webapp/frontend/README.md` was stale enough to actively mislead** — claimed Admin has "8 tabs" (actually 14) and closed with "full Apps Script feature parity (Phases 1-15)" as if that were still current, with no mention of Home/Reminders/Call History/Customers or anything from Phase 22 onward. Rewrote the stale claims to point at the actual source of truth (`Admin.tsx`'s own tabs array, this file) instead of hardcoding numbers that will just rot again.
5. **Both webhook shared-secret checks used a plain `!==`** (`routes/messaging.ts:122`, `routes/phase22.ts:242`) — a low-severity but free-to-close timing side-channel on a low-entropy secret. Added `timingSafeEqual()` (`lib/auth.ts`) and switched both checks to it, with a dedicated regression test.
6. **One more unbatched per-agent lookup** — `Phase14Api.getDashboardMetrics`'s `byAgent` breakdown (`phase14Api.ts:86-90`) called `users.get()` once per distinct assigned agent instead of one `users.list()` + Map, the same pattern already fixed elsewhere. Batched it.

**Also went further than the audit's minimum ask on two bigger items, since "based on rating work on missing points" reads as fix-not-just-list:**

- **Frontend had zero test infrastructure at all** (audit's single largest gap — a 4,800-line React app with no automated coverage). Added `vitest` + `@testing-library/react` + `jsdom` (`vite.config.ts`, `npm run test`), plus real regression tests: `Sidebar.test.tsx` pins down exactly which roles see Home/Dashboard/Admin (guards against the role-unaware-default-page bug class from the Home page work above ever coming back silently), `Charts.test.tsx` covers `PieChart`/`BarChart`'s empty-state and rendering logic. 8 tests, all passing. This is a genuinely minimal start, not comprehensive coverage — a real remaining gap, just no longer a zero.
- **The 608.89 kB single-chunk bundle warning** — code-split `Admin` (1,299 lines, ADMIN-only), `Dashboard`, and `Home` out of the initial bundle via `React.lazy`/`Suspense` in `App.tsx`, since all three are already gated behind their own role checks and a plain AGENT (the common case) never needs to fetch them. Main chunk dropped to 555.64 kB; `Admin` alone became its own 41 kB chunk fetched only when an admin actually navigates there. Verified live — fetched each new chunk's real URL directly in production and confirmed it serves real, correctly-bundled JS (not a 404 or the SPA HTML fallback).

**Deliberately left for you to weigh in on, not fixed unilaterally**: the round-robin assignment race condition (`phase7Api.ts:173-183`, `phase22Api.ts:901-914` — two leads/conversations landing at the same instant can both get assigned to the same agent) is a documented, *accepted* trade-off already (see `repository.ts:8-16`'s own comment), not an oversight — worth revisiting if lead volume grows, but not something to silently change the architecture for overnight. Also not done: tests for the two zero-coverage backend services (`realtimeListenApi.ts`, `metaAdsProvider.ts`) — flagged, not started.
...
**Built the missing half.** `ChatbotIntegrationApi.notifyInboundMessage` now POSTs every inbound message (while a number's mode is `active` or `shadow`, and the conversation isn't already handed off to a human) straight to that number's configured webhook URL — JSON body with the conversation/message/customer context your bot needs to act, signed `X-Chatbot-Signature: sha256=<hmac>` (same convention GitHub/Stripe webhooks use) so your FastAPI server can verify a call is really from us before trusting it. The signing secret is generated alongside the existing API key (Admin → Chatbot → rotate), shown once, same as the key already was — give both to your bot team together. Deliberately best-effort/non-blocking, same principle as the Lead auto-creation and Zoho sync: a chatbot outage or bad URL can never stop a real WhatsApp message from being recorded, it just logs a `WEBHOOK_FAILED` activity instead.

Admin → Chatbot now documents both directions in place: the outbound payload/signature contract, and the existing inbound callback contract (corrected while I was in there — it's actually `Authorization: Bearer <key>`, not the `X-Api-Key` header the UI never actually said before I added this section). 3 new backend tests (signed delivery + payload shape, non-blocking on failure, correctly silent when mode is off or a human already has the conversation). 273 backend tests + 8 frontend tests passing, deployed.

**What this means for your team**: once you set a number to `active` or `shadow` mode with a webhook URL in Admin → Chatbot and rotate a key, your FastAPI server will start receiving real inbound-message POSTs for that number and can reply via the existing callback endpoint — no more manual/placeholder step, this is the live wire. Recommend starting a real number in `shadow` mode first (drafts get logged as activity, nothing actually sends) to confirm your bot's parsing/signature verification against real traffic before flipping to `active`.

**Not yet live-tested against your real FastAPI server** — this is thoroughly covered by tests against the exact contract, but the true end-to-end proof is your bot receiving and correctly verifying a real signed webhook call. Worth doing together once your server is reachable.

## ✅ Templates isolated by WABA + unblocked a pre-existing Zoho queue (2026-08-31)

**Per your explicit ask**: agents composing on one number could see (and send!) another number's approved templates — `Phase10Api.listTemplates()` had zero scoping, returning every template in the system to any authenticated user, and the Inbox explicitly fetched it once and cached it across every number switch ("Quick replies and templates aren't number-scoped" — no longer true for templates). Fixed: `listTemplates` now takes an optional `wabaId` filter; the Inbox's Send Template picker passes the currently-open number's own `wabaId` and refetches on every number switch; Admin's template-management view still sees everything unfiltered, since managing the whole catalog is exactly what that page is for and it's already ADMIN-only. 1 new backend regression test proving two WABAs' templates stay properly separated. Backend/frontend both typechecked, 270 backend tests + 8 frontend tests passing, deployed. **Not yet click-through-verified in the browser** — the Chrome extension this session uses for live verification disconnected right as I went to check; the logic itself is proven by the new test, but seeing the actual Send Template dropdown change per number is still worth a real look when you get a chance.

**Unrelated but discovered while deploying this**: the backend deploy failed outright with `Queue 'whatsapp-panel-zoho-customer-sync' does not exist` — a Zoho customer-sync queue that's been configured in `wrangler.toml` (from earlier Zoho integration work) but was never actually created in Cloudflare, so every deploy since that work landed would have failed the same way. Created both the queue and its dead-letter queue (hit one real Cloudflare Queues API quirk along the way: the installed wrangler version's default retention period, 345600s, is now rejected by the live API, which caps at 86400s — worked around with an explicit `--message-retention-period-secs 86400`). This means the Zoho customer-sync producer/consumer pairing is now actually live for the first time, not just present in code — worth knowing if you weren't expecting that to be active yet.

## ✅ Delete leads + every WhatsApp number now auto-creates a Lead (2026-08-24)

**Per your explicit follow-up requests**, after the ECHT Marine investigation above:

1. **You can now delete a lead.** Didn't exist at all before — no backend endpoint, no UI. Added `Phase22Api.deleteLead` (ADMIN/SITE_MANAGER only, same `LEADS_MANAGE` + location-isolation check every other lead mutation already uses), a `DELETE /api/leads/:id` route, and a "Delete lead" button in the Lead detail modal with a two-step confirm ("Delete this lead permanently?" → Cancel/Confirm) rather than a browser-native `confirm()` popup, to match this app's existing UI style. Deleting a lead also cleans up its remarks, its stage assignment, and any still-pending reminder — but deliberately leaves its Call History and Quotations alone, since those are real business history (a call that happened, a quote that may already be in a customer's inbox via its public link), not something that should silently vanish. Confirmed live: deleted a real test lead, confirmed it disappeared from the board after a fresh reload.

2. **Every new WhatsApp conversation now falls under a real Lead**, per your explicit ask and the choices you made when I asked: applies to all 10 numbers (not just customer-facing sites — added 3 more locations, `Compliances`/`Entartica Partner Desk`/`Entartica CRM`, so every registered number has a matching location, same naming convention as `ECHT Marine`); dedup is by phone number alone, broader than the existing manual-add dedup (phone+location) — a person who messages two different numbers/brands never gets two Lead records; deletion permission is ADMIN+SITE_MANAGER. New `Phase22Api.autoCreateLeadFromConversation`, called from `Phase4Api.ingestInboundMessage` on every brand-new conversation, same "system-triggered, no signed-in actor" pattern the existing round-robin auto-assignment on that same call site already uses. Deliberately best-effort/non-blocking — any failure here is caught and logged, never allowed to break message ingestion itself, the same principle `docs/ZOHO_PHASE_2.md` already states for the future Zoho sync. **Caveat, flagged honestly**: this is thoroughly covered by 4 new backend tests (fresh-customer creation + correct assignment, phone-based dedup across locations, no-duplicate-on-second-message, and graceful no-op if a number's location can't be resolved) but has **not yet been confirmed against a real live WhatsApp message** — the next real inbound message on a previously-lead-less phone number is the first true end-to-end proof; let me know when one comes in so I can confirm it worked.

256 backend tests passing (7 new), 8 frontend tests passing, clean typecheck both sides, deployed. Delete-lead and the new locations dropdown both confirmed live in the browser; auto-creation confirmed via tests only, pending its first real message.

## ✅ Real bug found and fixed — "ECHT Marine" leads invisible to Nishad (2026-08-24)

**Reported by you**: a new Agent (Nishad Nayak) added to ECHT Marine "unable to see any leads which is currently sitting there." Investigated against the real live data rather than guessing.

**What was actually going on — two separate, real things, not one bug:**

1. **"ECHT Marine" wasn't a valid Lead location at all.** Leads and WhatsApp Numbers are two separate systems in this app — Leads are scoped to a fixed list of locations (`Raipur, Rajsamand, Coimbatore, Prayagraj, Alibaug, Saraighat`), completely independent of WhatsApp Number names. Giving Nishad access to the ECHT Marine *number* (confirmed granted — his Inbox access is fine) has zero effect on Lead visibility. You confirmed ECHT Marine is a genuinely separate brand from Entartica Sea World, based at Head Office only — so this needed a real new location, not a re-mapping to an existing one. Added **"ECHT Marine"** as a 7th location (`Phase22Locations` in the backend, `LEAD_LOCATIONS` in the frontend — both are the single source of truth every dropdown/filter/report already reads from, so nothing else needed touching). Named to exactly match the WhatsApp Number's own display name so the existing manager-visibility resolution (`findNumberForLocation`) picks it up automatically, same as `Raipur` already resolves to `Entartica - Raipur`.

2. **A real, separate bug this surfaced**: trying to actually configure ECHT Marine's assignment rule failed with `VALIDATION_ERROR: location is invalid` even after the location was added correctly (confirmed via a direct API check that `Phase22Locations` did include it). Root cause: **itty-router doesn't URL-decode path segments** — `/api/locations/ECHT%20Marine/assignment-config` arrived at the validation code still literally containing `%20`, not a real space, so it could never match any location name in the list. This bug has existed since Phase 22 first shipped but was completely invisible until now, because every location added before today happened to be a single word with nothing to encode — "ECHT Marine" is the first one with a space in it. It was also invisible to all 249 backend tests, because every existing test calls the service classes directly and skips the HTTP routing layer entirely, where this bug actually lives. Fixed the shared `param()` helper (duplicated identically across 6 route files) to `decodeURIComponent` path segments, with a safe fallback if decoding ever fails. Added a new route-level test file (`routeParamDecoding.test.ts`) — the first test in this codebase that dispatches a real `Request` through the real router instead of calling a service class directly, specifically to catch this class of bug going forward.

**Configured live**: ECHT Marine's assignment rule is now Single agent → Nishad Nayak, Active — any new lead uploaded/added under that location will auto-assign to him from now on.

**One more thing worth knowing, not a bug**: there are currently **zero formal Lead records** under any location for ECHT Marine — the 3 leads that exist system-wide are all under Raipur (test data + your own). If "leads sitting there" means the 79 real WhatsApp conversations on the ECHT Marine number, those are Conversations, not Leads — two separate entities in this system, connected only by a best-effort phone-number match. They'd need to be added as real Lead records (bulk CSV upload or "+ Add lead") before Nishad's new assignment rule has anything to actually assign.

249 backend tests passing (3 new), clean typecheck both sides, deployed and live-verified end to end — including directly confirming the exact failing request before the fix and the exact same request succeeding after.

## ✅ Firebase RTDB → D1 migration, Phase 1 of 6: foundations (zero behavior change) (2026-08-24)

**Per your explicit "switch Firebase RTDB to Cloudflare D1"** — after rating the stack 7/10 and calling RTDB out as the real structural weak point (no server-side filtered queries, root cause of at least 4 separate Cloudflare subrequest-limit bugs this project has hit). This is a live production system with real business data, so the approach is a full phased plan (saved, and summarized in the plan file referenced from this repo) rather than a single risky cutover: dual-write validation, staged per-collection cutover starting with the lowest-risk data, and a staging D1 database for all testing before production D1 is ever touched. You approved the plan and its key decisions (thin Firebase slice kept just for realtime chat push; one D1 table per collection; staging-first; 1-week validation window per collection; per-collection staged cutover, not one global switch).

**What shipped tonight is Phase 1 of 6 — foundations, deliberately zero behavior change.** A staging D1 database now exists (`whatsapp-panel-db-staging`) with all 34 tables created (`migrations/0001_init.sql`), each with `id`/`data` (full record as JSON) plus real indexed columns for whichever fields are actually filtered/queried on today (`numberId`, `status`, `customerId`, etc. — see `src/lib/d1Schema.ts`, the single source of truth the SQL, the store layer, and future backfill/parity scripts all read from). New `AppDb` class (`src/lib/appDb.ts`) sits underneath every `Repository<T>` instead of a raw `FirebaseDb` — it holds a real Firebase connection permanently (needed forever for realtime chat push and `mintCustomToken`) plus an optional D1 binding and a **per-collection** mode (`'firebase'`/`'dual'`/`'d1'`, defaulting to `'firebase'` for anything not explicitly listed). Because `Repository<T>` only ever called `get/put/patch/delete/list` on its db, this swap needed a type-annotation change across `repository.ts` and ~20 service files' constructors — mechanical, verified by a clean `tsc --noEmit` — and zero changes to any of the ~81 actual `Repository` call sites or their business logic.

**Confirmed zero behavior change, not just claimed**: all 237 pre-existing backend tests pass completely unmodified (nothing in them changed), plus 9 new tests (`test/d1Repository.test.ts`, against a new hand-rolled `mockD1.ts` test harness) proving the D1 path itself works correctly — create/get/list/update/remove/replace/count/findOne, 409/404 parity with the Firebase-backed path, indexed columns populated correctly, and a smoke test round-tripping all 34 tables through the shared generic mechanism (proving the schema-driven design actually works for every collection, not just a couple of hand-picked ones). Also sanity-checked the exact SQL `d1Store.ts` generates against the real staging D1 (not just the mock) — a real `INSERT OR REPLACE`/`SELECT`/`DELETE` round-trip. Deployed to production with the staging D1 binding attached (nothing reads/writes it yet — `DATA_BACKEND_MODES` is deliberately unset, so every collection still defaults to `'firebase'`) and live-verified: Home page after this deploy shows the exact same real numbers as before (3 leads, ₹119,070 pipeline, same quotation breakdown) — the AppDb refactor changed nothing a user can observe.

**What's next (Phases 2-6, not started)**: build the realtime-chat-ping slice (thin Firebase footprint, independent of the rest), real staging-D1 smoke testing for the backfill/comparison scripts specifically, then the first staged cutover starting with low-risk collections (`adAccounts`, `quickReplies`), working up to `leads`/`quotations`/`webapp_messages` last — each with its own 1-week dual-write validation window. Full detail, including the exact schema, every new/changed file, and the explicit out-of-scope items (real filtered SQL queries, the round-robin race condition, migrating the test runner to Miniflare) is in the approved migration plan.

## ✅ New "Home" page — cross-location overview with pie/bar charts (2026-08-24)

**What it is, per your explicit "work on Home Page... different from Dashboard, showcase Pie chart, Bar Chart and Graphs for important factors."** Dashboard stays exactly what it was — per-number conversation operations (open/needs-reply/unassigned, agent workload, response time). Home is new: a business-wide strategic view across every location/number you can access, covering three things Dashboard never touched — where leads are actually coming from, whether placed calls connect, and how much is sitting in the sales pipeline.

**Backend**: new `Phase22Api.getHomeMetrics()` (`GET /api/home-metrics`, gated `REPORTS_VIEW`, same permission Dashboard already requires) returns three series: leads-by-location (bar chart), call outcomes bucketed Answered/Missed/Pending (pie chart, lead-initiated calls only for now — see caveat below), and quotation pipeline value by status (table). Reuses `listLeads()`'s existing manager/agent + location authorization wholesale, so Home can never show a signed-in user more than they could already see elsewhere. 5 new backend regression tests (234 total, all passing).

**Frontend**: new `Home.tsx` (4 KPI cards + the three charts) and `Charts.tsx` (extracted `PieChart`/`BarChart` out of `Dashboard.tsx` byte-for-byte, so both pages share one chart implementation instead of duplicating it). New 🏠 "Home" nav item in the sidebar, gated the same way as the existing Dashboard link (SUPERVISOR/SITE_MANAGER/VIEWER/ADMIN — anyone with `REPORTS_VIEW`).

**A real pre-existing gap found and fixed while wiring this up, not introduced by Home**: the app's default landing page was hardcoded to `'dashboard'` for every signed-in user with zero role-awareness — meaning a plain AGENT (who, per `Phase14Api`'s own doc comment, does not have `REPORTS_VIEW`) would sign in and land directly on a page that immediately 403s. This bug already existed before today; Home just made it visible again by needing the same fix. Now the app defaults to `'home'` and a `useEffect` keyed on `whoAmI` redirects anyone without `REPORTS_VIEW` straight to Inbox (a page every role can see) the moment their role is known — so both Home and Dashboard are covered by the same fix.

**Caveat, flagged honestly**: the "Call outcomes" chart only counts calls placed directly from a Lead (`CallLog.leadId` set) — conversation-initiated calls aren't represented yet, to avoid a real scoping bug (a naive "all calls this user can see" query would leak calls across a SITE_MANAGER's location boundary, since `CallLog` has no location field of its own). The doc comment on `getHomeMetrics()` spells out the real fix once conversation-initiated calls need representing too: scope by `Phase5Api.listMyNumbers()` the same way `Phase14Api.getDashboardMetrics` already does.

Typecheck clean, `npm run build` clean, both backend and frontend deployed and confirmed reachable live (console-clean page load) — the interactive KPI/chart rendering itself needs your own sign-in to fully confirm, since Google Workspace SSO isn't something this session can drive on your behalf.
